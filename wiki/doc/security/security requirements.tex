...
\item Checkpointed System will be selected, even though it has a negative influence on both the Cost and the Performance,
...
The Checkpointed System pattern is selected and instantiated to deal with the Application Integrity requirement.

We can now consider the Application Integrity requirement as resolved at the architectural level.
...
Checkpointed System has already been instantiated when Application Integrity was considered.
...
makes it also easy to distribute the load e.g. different tables can be located at different servers and the lobby
...
So Load Balancer is selected and instantiated to deal with the Availability requirement.
...
So if we decided to select and instantiate Container Managed Security we could backtrack over the choices we made
after the selection of Authentication and Authorization Enforcer.

Container Managed Security has as labels +Manageability, +Portability, +Integrity and is an alternative for
...
and Authorization Enforcer offer more benefits.
...
Controlled Process Creator isn't applicable because there is a thread pool with worker threads, that limits the
number of concurrent processes.
Controlled Object Factory depends on Controlled Object Monitor, which was discarded earlier.

Application Firewall has as labels -Performance, +Manageability, +Auditability and has as alternative Input Guard.
Demilitarized Zone has as labels -Performance, -Cost, -Manageability, -Dependability, +Maintainability and depends
on Firewall.
Firewall has as labels +Accountability, -Performance, -Dependability and benefits from Demilitarized Zone and 
Application Firewall.

Application Firewall is selected and instantiated because auditability is an important quality label compared to
the performance penalty.
Demilitarized Zone isn't selected because the cost of building one would be less than the cost of an actual intrusion
and a demilitarized zone doesn't seem useful if the only task of the front end is to forwards request.
...
Audit Interceptor has as labels -Performance, +Auditability, +Maintainability, +Manageability and depends on
...
Secure Logger is already dealth with because the latter pattern was already present.
...
Secure Message Router has as labels +Manageability, +Maintainability, +Confidentiality, +Integrity, -Availability
