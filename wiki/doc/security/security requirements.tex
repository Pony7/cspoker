...
By following the steps in the pattern-based approach described in the previous section the vanilla architecture has been enhanced with security related components. The decomposition view can be found in figure \ref{fig:decomposition_secured} together with the deployment view in figure \ref{fig:deployment_secured}. Secure pipes are used to protect all actions and events from both eavesdropping and modification.  The incoming actions pass through the application firewall where all input validation is done. This is essential to counter tampering. All the actions pass through the secure service facade from the corresponding component where authentication and authorization is done by communicating with the account server. The different types of credentials are encapsulated in by the credential tokenizer. The firewall is used to counter denial of service attacks and block connections from malicious clients to decrease the load of the main server. Load balancing is done between all the table servers by the lobby server (not considered in the analysis). The number of table servers can be extended easily, since all tables are independent. The audit interceptor is used to delegate all logging needs for auditing to the secure logger. The secure access layer is used to cope with future changes in the persistence service. 
