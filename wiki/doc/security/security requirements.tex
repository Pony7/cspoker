...
\textbf{Alternative paths:}
\begin{itemize}
\item The mis-actor performs a replay from an action of an other actor. 
\item The mis-actor tampers with logging to remove evidence. 
\item The mis-actor spoofs an other actor.
\end{itemize}
\textbf{Capture points:}
\begin{itemize}
\item \textbf{Prevention:}
...
\item Log all actions in a secure way.
\item Strong authentication to make spoofing impossible. 
\end{itemize}
\end{itemize}
...
security analysis, and to not got into depth in the sockets or http protocol.
