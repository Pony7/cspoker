...
\item The mis-actor changes messages to his advantage, sends fake messages that go undetected or replays messages sent by a Player (For example: the mis-actor replaces the bet amount by a larger number when he has very good cards)
\end{itemize}
\textbf{Alternative paths:}
\begin{itemize}
...
\item The mis-actor changes events to his advantage, sends fake events or replays events (For example: the mis-actor replaces the pocket cards sent to another player by aces to trick him in to betting excessively).
\end{itemize}
\textbf{Alternative paths:}
\begin{itemize}
\item The mis-actor violates the integrity of the channel to tamper with the data flow.
