...
Checkpointed system & S4 & & & NR4 \\\hline
...
\item \textbf{T1:} Choosing not to replicate the system, is based on a tradeoff between cost and fault tolerance.
...
\textbf{Architectural Decision} & \textbf{Sensitivity} & \textbf{Tradeoff} & \textbf{Risk} & \textbf{Nonrisk}\\\hline
Broker & & & & NR1 \\\hline
Publish-subscriber & & & R2& \\\hline
Message queueing & & & & NR3  \\\hline
Load balancer & & & & NR4  \\\hline
No replicated system & & & R5 &  \\\hline
...
things can be switched and adapted without the clients noticing it.
\item \textbf{R2:} With the publish-subscriber approach, there may be events that are lost during transmission due
to a network failure. Recovery has to deal with these missing events.
\item \textbf{NR3:} Recovery from a network failure is easier with a message queue, because the messages can just
be queued further untill the network problem is resolved.
...
\item \textbf{R5:} If there is full replication of the system, recovery would just consist of swapping the existing system
with it's replica. So not having replication holds a risk.
