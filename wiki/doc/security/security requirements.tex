...
\item Secure Logger: the Logging Engine takes the task of a Secure Logger.
\end{itemize}
...
Next, we screen the short descriptions of the selected patterns to see if they are applicable. Security Association
appears to be missing from the list and is thus discarded. Session won't be selected, because there is too 
little shared state between the different processes; the state is almost only shared within a process.The other 
patterns seem applicable.
...
than Single Access Point, and it benefits from 2 patterns that already exist in the architecture (Secure Pipe and
Secure Service Facade).Single Access Point would also lead to a too centralised architecture.Container Managed 
