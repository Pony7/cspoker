...
Spoofing the logging engine & - \\\hline
...
Repudiation of using the logging engine & - \\\hline
...
Repudiation of logging something in the audit log & - \\\hline
...
A malicious player uses multiple accounts to access more hidden cards & \\\hline
A malicious player with admin rights misuses his privileges & \\\hline
TODO & \\\hline
\end{tabular}
\end{center}
...
From the business goals identified in the initial architecture, the following important quality attributes are selected:  usability, security and availability. The overview of the quality attribute tree can be found in table \ref{QAT}. In the following sections the rationale for choosing the importance (business perspective) and difficulty (technical perspective) ratings are elaborated.
\begin{table}[htpb]
\begin{center}
\begin{tabular}{| p{2,25cm} | p{4,25cm} | p{5cm} | p{0,4cm} | p{0,4cm} | p{0,4cm} |}
\hline
\textbf{Level 2: Quality Attribute} & \textbf{Level 3: Quality Attribute Refinement} & \textbf{Level 4: Quality Attribute Scenario} & \textbf{I} & \textbf{D} & \textbf{S}\\
\hline
\hline
\textbf{Availability} & Prevention & Network failure occurence & H & M & H  \\\cline{3-6}
& & Hardware failure occurence & H & M & H \\\cline{2-6}
& Fault detection & Detection of network failure & M & M & M \\\cline{3-6}
& & Detection of hardware failure & M & M & M \\\cline{2-6}
& Recovery & Network failure recovery & M & H & H \\\cline{3-6}
& & Hardware failure recovery & M & H & H \\\hline
\textbf{Security} & S1 Authentication & S1.1 99.999\% of all users are correctly authenticated & H & M & H  \\\cline{2-6}
& S2 Authorization & S2.1 99.999\% of all user actions are correctly authorized & H & M & H \\\cline{2-6}
& S3 Storage Confidentiality & S3.1 All account related data are secure 99.999\% of time & H & L & L  \\\cline{2-6}
& S4 Transmission Confidentiality & S4.1 Cards are transmitted secure 99.999\% of time & M & L & L \\\cline{2-6}
& S5 Transmission Integrity & S5.1 99.999\% of all transmitted cards remain unmodified & M & L & L \\\cline{3-6}
& & S5.2 99.999\% of all transmitted actions remain unmodified & M & L & L \\\cline{2-6}
& S6 Auditing & S6.1 99.999\% of actions are logged and kept for 2 years & L & L & L \\\hline
\textbf{Usability} & & User must be able so seat within 3 clicks & L & L & L \\\cline{3-6}
& & User must be able to find the correct button within 2 seconds & H & L & L \\\cline{3-6}
& & User must be able to define default actions & M & L & L \\\cline{3-6}
& & The user interface must respond to an action within 500ms & H & M & H \\\cline{3-6}
& & The user interface must be separated to enable easy testing & M & M & M \\\hline
...
\subsection{Security}
\begin{itemize}
\item S1.1 99.999\% of all users are correctly authenticated
\begin{itemize}
\item Importance: 
\item Difficulty:
\end{itemize}
\item S2.1 99.999\% of all user actions are correctly authorized
\begin{itemize}
\item Importance: H, it's important users can only do what they are allowed to do.
\item Difficulty: M, 
\end{itemize}
\item S3.1 All account related data are secure 99.999\% of time
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
\item S4.1 Cards are transmitted secure 99.999\% of time
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
\item S5.1 99.999\% of all transmitted cards remain unmodified
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
\item S5.2 99.999\% of all transmitted actions remain unmodified
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
\item S6.1 99.999\% of actions are logged and kept for 2 years
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
...
\begin{itemize}
\item User must be able to seat with 3 clicks
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
\item User must be able to find the correct button within 2 seconds
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
\item User must be able to define default actions (e.g. default check/fold)
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
\item The user interface must respond to an action within 500ms
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
\item The user interface must be separated to enable easy testing
\begin{itemize}
\item Importance:
\item Difficulty:
\end{itemize}
...
\textbf{Reasoning} & \multicolumn{4}{| p{9cm} |}{reasoning} \\\hline
...
\textbf{Response} & \multicolumn{4}{| p{9cm} |}{Authorization is performed before every sensitive operation. The authorization service is available for 99.999\% of all user actions.} \\\hline \hline
\textbf{Architectural Decision} & \textbf{Sensitivity} & \textbf{Tradeoff} & \textbf{Risk} & \textbf{Nonrisk}\\\hline
...
\textbf{Reasoning} & \multicolumn{4}{| p{9cm} |}{All actions pass through the secure service facade where the user is authenticated with the authentication enforcer. The authorization is performed on the authenticated subject by the authorization enforcer.} \\\hline
