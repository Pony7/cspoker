...
public abstract class Expander {
	...
	protected Set<ProbabilityAction> getProbabilityActions() {
		...
		if (gameState.hasBet()) {
			// call, raise or fold
			Triple<Double, Double, Double> probabilities = node
					.getFoldCallRaiseProbabilities();

			double foldProbability = probabilities.getLeft();
			totalProbability += foldProbability;
			actions.add(new ProbabilityAction(new FoldAction(gameState, node
					.getPlayerId()), foldProbability));

			double callProbability = probabilities.getMiddle();
			totalProbability += callProbability;
			actions.add(new ProbabilityAction(new CallAction(gameState, node
					.getPlayerId()), callProbability));

			if (!gameState.getPlayer(node.getBotId()).isAllIn()
					&& gameState.isAllowedToRaise(node.getPlayerId())) {
				double raiseProbability = probabilities.getRight();
				int lowerRaiseBound = gameState.getLowerRaiseBound(node
						.getPlayerId());
				...
				actions.add(new ProbabilityAction(new RaiseAction(gameState,
						node.getPlayerId(), lowerRaiseBound), raiseProbability
						/ n));
			...
			// check or bet
			Pair<Double, Double> probabilities = node
					.getCheckBetProbabilities();
			double checkProbability = probabilities.getLeft();
			totalProbability += checkProbability;
			actions.add(new ProbabilityAction(new CheckAction(gameState, node
					.getPlayerId()), checkProbability));
				...
				if (n > 1) {
					...
					for (double betSizeSample : betSizeSamples) {
						BetAction betAction = new BetAction(gameState, node
								...
								.round(lowerRaiseBound + betSizeSample
										* (upperRaiseBound - lowerRaiseBound)));
