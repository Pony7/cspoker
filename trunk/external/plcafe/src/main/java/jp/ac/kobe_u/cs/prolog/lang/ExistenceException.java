...
public class ExistenceException extends BuiltinException {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6720389434667194931L;

	/** A functor symbol of <code>existence_error/5</code>. */
	public static SymbolTerm EXISTENCE_ERROR = SymbolTerm.makeSymbol(
			"existence_error", 5);
	...
	/**
	 * Constructs a new <code>ExistenceException</code> with the given
	 * arguments.
	 */
	public ExistenceException(Predicate _goal, int _argNo, String _objType,
			Term _culprit, String _message) {
		goal = _goal;
		argNo = _argNo;
		objType = _objType;
		culprit = _culprit;
	...
	public Term getMessageTerm() {
		Term[] args = { new JavaObjectTerm(goal), new IntegerTerm(argNo),
				SymbolTerm.makeSymbol(objType), culprit,
