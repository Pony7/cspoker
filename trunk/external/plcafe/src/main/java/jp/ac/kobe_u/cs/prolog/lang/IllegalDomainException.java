...
public class IllegalDomainException extends BuiltinException {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5335987719856993959L;

	/** A functor symbol of <code>domain_error/4</code>. */
	public static SymbolTerm DOMAIN_ERROR = SymbolTerm.makeSymbol(
			"domain_error", 4);

	/*
	 * domain ::= character_code_list | close_option | flag_value | io_mode |
	 * non_empty_list | not_less_than_zero | operator_priority |
	 * operator_specifier | prolog_flag | read_option | source_sink | stream |
	 * steam_option | stream_or_alias | stream_position | stream_property |
	 * write_option | hash_or_alias| hash_option | 'arithmetic expression'
	 */
	...
	/**
	 * Constructs a new <code>IllegalDomainException</code> with a valid domain
	 * and its culprit.
	 */
	 ...
	 * arguments.
	 */
	public IllegalDomainException(Predicate _goal, int _argNo, String _domain,
			Term _culprit) {
		goal = _goal;
		argNo = _argNo;
		domain = _domain;
	...
	public Term getMessageTerm() {
		Term[] args = { new JavaObjectTerm(goal), new IntegerTerm(argNo),
				SymbolTerm.makeSymbol(domain), culprit };
		return new StructureTerm(DOMAIN_ERROR, args);
	}

	/**
	 * Returns a string representation of this
	 * <code>IllegalDomainException</code>.
	 */
	@Override
