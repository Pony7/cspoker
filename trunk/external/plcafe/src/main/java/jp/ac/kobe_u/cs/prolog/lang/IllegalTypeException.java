...
public class IllegalTypeException extends BuiltinException {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4432713530734665999L;

	/** A functor symbol of <code>type_error/4</code>. */
	public static SymbolTerm TYPE_ERROR = SymbolTerm
			.makeSymbol("type_error", 4);

	/*
	 * type ::= atom | atomic | byte | callable | character | compound |
	 * evaluable | in_byte | in_character | integer | list | number |
	 * predicate_indicator | variable | flaot | java
	 */
	...
	/**
	 * Constructs a new <code>IllegalTypeException</code> with a valid type and
	 * its culprit.
	 */
	...
	/**
	 * Constructs a new <code>IllegalTypeException</code> with the given
	 * arguments.
	 */
	public IllegalTypeException(Predicate _goal, int _argNo, String _type,
			Term _culprit) {
		goal = _goal;
		argNo = _argNo;
		type = _type;
	...
	public Term getMessageTerm() {
		Term[] args = { new JavaObjectTerm(goal), new IntegerTerm(argNo),
				SymbolTerm.makeSymbol(type), culprit };
