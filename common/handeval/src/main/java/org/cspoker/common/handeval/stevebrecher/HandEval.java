...
public final class HandEval {
	...
	/**
	 * Calculate the rank of the given hand. Larger numbers are better.
	 * 5-, 6- or 7-card hands are supported.
	 */
	public static int getRank(EnumSet<Card> cards) {
		...
		if (nbCards == 7) {
			...
			throw new IllegalArgumentException("Unsupported number of cards: "
					+ nbCards);
		}
	}
	...
	private static final int THREE_OF_A_KIND = HandCategory.THREE_OF_A_KIND
			.ordinal() << VALUE_SHIFT;
	...
	private static final int FOUR_OF_A_KIND = HandCategory.FOUR_OF_A_KIND
			.ordinal() << VALUE_SHIFT;
	private static final int STRAIGHT_FLUSH = HandCategory.STRAIGHT_FLUSH
			.ordinal() << VALUE_SHIFT;

	/* Arrays for which index is bit mask of card ranks in hand: */
	private static final int ARRAY_SIZE = 0x1FC0 + 1; // all combos of up to 7
														// of LS 13 bits on
	...
	private static final int[] straightValue = new int[ARRAY_SIZE]; // STRAIGHT
																	...
																	// 0 if no
																	// straight
	private static final int[] nbrOfRanks = new int[ARRAY_SIZE]; // count of
																	// bits set
	...
	private static final int[] loEvalOrNo8Low = new int[ARRAY_SIZE]; // 5 bits
																		// set
																		// in LS
																		// 8
																		// bits,
	...
	private static int flushAndOrStraight7(final int ranks, final int c,
			final int d, final int h, final int s) {
	...
	public static int hand7Eval(long hand) {
		...
		/*
		 * The low-order 52 bits of hand contains four 13-bit fields, one field
		 * per suit. The high-order 12 bits are clear. Get the respective fields
		 * into variables. We don't care which suit is which; we arbitrarily
		 * call them c,d,h,s.
		 */
		...
		case 3:
			/*
			 * trips and pair (full house) with non-playing pair, or two trips
			 * (full house) with non-playing singleton, or quads with pair and
			 * singleton
			 */
		...
		case 4:
			/*
			 * three pair and singleton, or trips and pair (full house) and two
			 * non-playing singletons, or quads with singleton kicker and two
			 * non-playing singletons
			 */
			i = c ^ d ^ h ^ s; // the bit(s) of the trips, if any, and
								// singleton(s)
			if (nbrOfRanks[i] == 1) {
				...
				return hiTopRankTWO_PAIR[ranks] | hiBotRank[j]
						| hiRankMask[hiRankMask[j] ^ j | i];
			}
			if ((j = c & d & h & s) == 0) {
		...
		case 5:
			/*
			 * flush and/or straight, or two pair and three singletons, or trips
			 * and four singletons
			 */
			...
			if (nbrOfRanks[i] != 5) {
				...
				return hiTopRankTWO_PAIR[j] | hiBotRank[hiRankMask[j] ^ j]
						| hiRankMask[i];
			}
		...
		case 6:
			/*
			 * flush and/or straight, or one pair and three kickers and two
			 * nonplaying singletons
			 */
			if ((i = flushAndOrStraight7(ranks, c, d, h, s)) != 0) {
				return i;
			}
	...
	public static int handRazzEval(long hand) {
		...
		final int c = (((int) hand & 0x1FFF) << 1)
				+ (((int) hand ^ 0x1000) >> 12);
		final int d = ((int) hand >> 12 & 0x3FFE)
				+ (((int) hand ^ 0x1000 << 13) >> 25);
		final int h = ((int) (hand >> 25) & 0x3FFE)
				+ (int) ((hand ^ 0x1000L << 26) >> 38);
		final int s = ((int) (hand >> 38) & 0x3FFE)
				+ (int) ((hand ^ 0x1000L << 39) >> 51);
		...
		case 3:
			/*
			 * AAABBBC -- two pair, AAAABBC -- two pair, AAABBCC -- two pair w/
			 * kicker = highest rank.
			 */
			/* bits for singleton, if any, and trips, if any: */
			if (nbrOfRanks[i = c ^ d ^ h ^ s] == 3) {
				...
				return hiTopRankTWO_PAIR[i] | hiBotRank[i ^ hiRankMask[i]]
						| ranks ^ i;
			}
		...
		case 4:
			/*
			 * AABBCCD -- one pair (lowest of A, B, C), AAABBCD -- one pair (A
			 * or B), AAAABCD -- one pair (A)
			 */
			i = c ^ d ^ h ^ s; /*
								 * the bit(s) of the trips, if any, and
								 * singleton(s)
								 */
			if (nbrOfRanks[i] == 1) {
	...
	private static int flushAndOrStraight6(final int ranks, final int c,
			final int d, final int h, final int s) {
	...
	public static int hand6Eval(long hand) {
		...
		case 2: /*
				 * quads with pair kicker, or two trips (full house)
				 */
		...
		case 3: /*
				 * quads with singleton kicker and non-playing singleton, or
				 * full house with non-playing singleton, or two pair with
				 * non-playing pair
				 */
			if ((c ^ d ^ h ^ s) == 0) {
		...
		case 4: /*
				 * trips and three singletons, or two pair and two singletons
				 */
			if ((i = c ^ d ^ h ^ s) != ranks) {
		...
		case 5: /*
				 * flush and/or straight, or one pair and three kickers and one
				 * non-playing singleton
				 */
	...
	public static int hand5Eval(long hand) {
		...
		case 2: /* quads or full house */
			...
			if ((i & h & s) == 0) { /* no bit common to all suits */
				...
				/*
				 * the quads bit must be present in each suit mask, but the
				 * kicker bit in no more than one; so we need only AND any two
				 * suit masks to get the quad bit:
				 */
				return FOUR_OF_A_KIND | hiBotRank[i] | i ^ ranks;
			}

		case 3: /*
				 * trips and two kickers, or two pair and kicker
				 */
			if ((i = c ^ d ^ h ^ s) == ranks) {
	...
	public static int ranksMask(long hand) {
		...
		return (int) hand & 0x1FFF | (int) hand >>> 13 & 0x1FFF
				| (int) (hand >>> 26) & 0x1FFF | (int) (hand >>> 39);
	}
	...
	// /**
	// * Returns the parameter with all bits except its highest-order bit
	// cleared.
	// * @param mask an int in the range 0..0x1FC0 (8128).
	// * @return the parameter with all bits except its highest-order bit
	// cleared.
	...
	//
	// /**
	// * Returns the rank (2..14) corresponding to the high-order bit set in
	// mask.
	// * @param mask an int in the range 0..0x1FC0 (8128).
	// * @return the rank (2..14) corresponding to the high-order bit set in
	// mask.
	// * @throws IndexOutOfBoundsException if mask < 0 || mask > 0x1FC0.
	// */
			...
			for (i = ACE_RANK - 1; i > 0; --i, shiftReg <<= 1) {
				if ((shiftReg & 0x1000) != 0) {
					...
					case 1:
						...
						break;
					case 2:
						hi2RanksMask[mask] = (shiftReg & 0x03FFF000) >> ACE_RANK
								- 1 - i;
						break;
					case 3:
						hi3RanksMask[mask] = (shiftReg & 0x03FFF000) >> ACE_RANK
								- 1 - i;
						break;
					case 5:
						hi5RanksMask[mask] = (shiftReg & 0x03FFF000) >> ACE_RANK
								- 1 - i;
					}
				}
			}
