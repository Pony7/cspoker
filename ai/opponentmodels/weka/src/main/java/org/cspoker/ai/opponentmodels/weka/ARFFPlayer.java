...
public class ARFFPlayer {
	...
	public ARFFPlayer(Object player, WekaRegressionModel baseModel) throws IOException {
		this.player = player;
		...
		if (!modelCreated || (modelCreated && WekaOptions.isContinueAfterCreation())){
			...
			preFoldCallRaiseFile = new ARFFFile(path, player, "PreFoldCallRaise.arff", 
					ARFFPropositionalizer.getPreFoldCallRaiseInstance().toString(), overwrite);
			postFoldCallRaiseFile = new ARFFFile(path, player, "PostFoldCallRaise.arff", 
					ARFFPropositionalizer.getPostFoldCallRaiseInstance().toString(), overwrite);
	...
	public void writePostCheckBet(Instance instance) {
		if (postCheckBetCreated && !WekaOptions.isContinueAfterCreation()) return;
	...
	public void writePreFoldCallRaise(Instance instance) {
		if (preFoldCallRaiseCreated && !WekaOptions.isContinueAfterCreation()) return;
	...
	public void writePostFoldCallRaise(Instance instance) {
		if (postFoldCallRaiseCreated && !WekaOptions.isContinueAfterCreation()) return;
