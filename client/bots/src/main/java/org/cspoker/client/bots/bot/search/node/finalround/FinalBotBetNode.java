...
public class FinalBotBetNode extends BotBetNode{
	...
	public FinalBotBetNode(PlayerId botId, GameState gameState, AllPlayersModel opponentModeler, int depth) {
		super(botId,gameState, opponentModeler, depth);
	}

	protected double doNextPlayer(GameState newGameState, PlayerState nextToAct) {
		ActionNode nextNode = new FinalOpponentBetNode(playerId, nextToAct.getPlayerId(),newGameState, opponentModeler, depth+1);
		nextNode.expand();
