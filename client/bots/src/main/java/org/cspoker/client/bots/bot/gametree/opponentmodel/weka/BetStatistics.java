...
public class BetStatistics implements Cloneable {
	...
	public float getRaiseFrequency(int memory) {
		...
		return (0.16F * memory + nbRaises)
				/ (memory + nbRaises + nbFolds + nbCalls);
	}
	...
	public float getRoundRaiseFrequency(Propositionalizer p, int memory) {
		if (p.inPreFlopRound()) {
			return (0.15F * memory + nbRaisesPreFlop)
					/ (memory + nbFoldsPreFlop + nbCallsPreFlop + nbRaisesPreFlop);
		} else if (p.inFlopRound()) {
			return (0.12F * memory + nbRaisesFlop)
					/ (memory + nbFoldsFlop + nbCallsFlop + nbRaisesFlop);
		} else if (p.inTurnRound()) {
			return (0.1F * memory + nbRaisesTurn)
					/ (memory + nbFoldsTurn + nbCallsTurn + nbRaisesTurn);
		} else if (p.inRiverRound()) {
			return (0.09F * memory + nbRaisesRiver)
					/ (memory + nbFoldsRiver + nbCallsRiver + nbRaisesRiver);
		} else {
			throw new IllegalStateException();
		}
	}
	...
	public float getAFq(int memory) {
		...
		float nbBets = getNbBets();
		float nbFolds = getNbFolds();
		return (0.24F * memory + nbRaises + nbBets)
				/ (memory + nbCalls + nbFolds + nbRaises + nbBets);
	}
	...
	public double getTotalCallAmount() {
		return callAmountPreFlop + callAmountFlop + callAmountTurn
				+ callAmountRiver;
	}

	public double getTotalRaiseAmount() {
		return raiseAmountPreFlop + raiseAmountFlop + raiseAmountTurn
				+ raiseAmountRiver;
	}

	public double getTotalBetAmount() {
		return betAmountPreFlop + betAmountFlop + betAmountTurn
				+ betAmountRiver;
	}
