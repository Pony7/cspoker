...
public class StructureTerm extends Term {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7378074777393851020L;

	/** Holds the functor symbol of this <code>StructureTerm</code>. */
	protected final SymbolTerm functor;
	...
	/**
	 * Constructs a new Prolog compound term such that <code>_functor</code> is
	 * the functor symbol, and <code>_args</code> is the argument terms
	 * respectively.
	 */
	public StructureTerm(SymbolTerm _functor, Term[] _args) {
		...
		if (arity != args.length) {
			throw new InternalException(
					"Invalid argument length in StructureTerm");
		}
	}
	...
	/*
	 * Term public boolean unify(Term t, Trail trail) { if (t.isVariable())
	 * return t.unify(this, trail); if (! t.isStructure()) return false; if (!
	 * functor.equals(((StructureTerm)t).functor())) return false; for (int i=0;
	 * i<arity; i++) { if (! args[i].unify(((StructureTerm)t).args[i], trail))
